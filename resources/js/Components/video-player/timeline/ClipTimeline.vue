    ...
    <div class="clip-timeline-panel">
        ...
        >
            <div class="header-content">
                ...
                <span class="header-title">Sincronizar Línea de Tiempo</span>
            </div>
        ...
        <transition name="slide-down">
            <div v-if="!isCollapsed" class="timeline-content">
                ...
                <TimelineOffset
                    ...
                    :initial-offset="videoStore.video?.timeline_offset || 0"
...
import TimelineOffset from './TimelineOffset.vue';
...
function getClipBlockStyle(clip: VideoClip) {
    // Use raw clip times without offset (clips created during playback already have correct timestamps)
    const startTime = parseFloat(clip.start_time as any) || 0;
    const endTime = parseFloat(clip.end_time as any) || 0;
    ...
    // Skip clips outside video duration
    if (startTime >= duration.value || endTime <= 0) {
        return {
            left: '0%',
            width: '0%',
            display: 'none',
    ...
    const startPercent = (startTime / duration.value) * 100;
    const widthPercent = ((endTime - startTime) / duration.value) * 100;
...
function getClipTooltip(clip: VideoClip): string {
    // Show raw times (clips already have correct timestamps)
    const startTime = parseFloat(clip.start_time as any) || 0;
    const endTime = parseFloat(clip.end_time as any) || 0;
...
function handleClipClick(clip: VideoClip) {
    // Use raw clip time (clips already have correct timestamps)
    const seekTime = parseFloat(clip.start_time as any) || 0;

    if (isFinite(seekTime) && seekTime >= 0) {
        console.log('Seeking to clip:', seekTime);
        ...
        console.error('Invalid seek time:', { clip, seekTime });
    }
}
...
function handleOffsetChanged(newOffset: number) {
    // Reload clips after offset changes
    clipsStore.loadClips(props.videoId);
}
</script>
