...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .share-header img { height: 28px; opacity: 0.9; }
        .share-header span { font-size: 13px; color: #666; }
        ...
        .replay-btn:hover { transform: scale(1.08); background: #00B7B5; }
        .replay-btn svg { width: 26px; height: 26px; fill: #fff; margin-left: 3px; }
        .replay-label { font-size: 13px; color: #ccc; font-weight: 500; }
...
<body>
    ...
    <div class="share-footer">
        <a href="{{ url('/') }}">RugbyHub · Análisis de video para rugby</a>
    </div>

    <script>
        @php
            $bunnyService = \App\Services\BunnyStreamService::forOrganization($video->organization);
            $hlsUrl = $video->bunny_video_id && $video->bunny_status === 'ready'
                ? $bunnyService->getHlsUrl($video->bunny_video_id)
                : ($video->bunny_hls_url ?? null);
        @endphp
        const hlsUrl    = @json($hlsUrl);
        const mp4Url    = @json($video->bunny_mp4_url);
        const streamUrl = @json(route('videos.stream', $video));
        ...
        const source = hlsUrl ?? mp4Url ?? streamUrl;
        ...
        if (hlsUrl && Hls.isSupported()) {
            ...
            loadingEl.innerHTML = '<p style="color:#888;font-size:13px">Video no disponible.</p>';
        }
    </script>
</body>
</html>
